...
use ::ores_web_app::ores_pages::__ores_page_src_pages_x_y_z_page_rs as __ores_page;
...
async fn __ores_handle_page(state: PageState, request: PageHttpRequest) -> Result<PageHttpResponse, RuntimeError> {
    invoke_page(request, state, ORES_PAGE_ROUTE, ORES_PAGE_AXUM_PATHS, |ctx| __ores_page::__ores_page_boxed(ctx)).await
}
