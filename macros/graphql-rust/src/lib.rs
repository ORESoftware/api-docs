...
fn validate(
    ...
    if item.sig.asyncness.is_none() {
        return Err(syn::Error::new_spanned(&item.sig.fn_token, "#[ores_graphql] requires an async resolver function"));
    }
    if matches!(&item.vis, Visibility::Inherited) {
        return Err(syn::Error::new_spanned(&item.vis, "#[ores_graphql] resolver must be pub or pub(crate) so generated schema glue can reference it"));
    }
    for input in &item.sig.inputs {
        let FnArg::Typed(typed) = input else {
            return Err(syn::Error::new_spanned(input, "#[ores_graphql] does not accept self receivers; keep resolver functions free-standing"));
        };
        if !matches!(typed.pat.as_ref(), Pat::Ident(_)) {
            return Err(syn::Error::new_spanned(&typed.pat, "#[ores_graphql] parameters must use simple identifier patterns"));
        }
    }
    ...
    for meta in args {
        let Meta::NameValue(value) = meta else {
            return Err(syn::Error::new_spanned(meta, "#[ores_graphql] arguments are operation_key = \"stable.key\", invoke = crate::...::__ores_invoke_fn, kind = \"query|mutation|subscription\", field = \"name\", stream = \"unary|server_stream\""));
        };
        if value.path.is_ident("operation_key") {
            set_once(&mut operation_key, string_value(&value.value, "operation_key")?, value, "operation_key")?;
    ...
    let operation_key = operation_key.ok_or_else(|| syn::Error::new_spanned(item, "#[ores_graphql] requires operation_key = \"stable.operation.key\""))?;
    ...
    let stream = stream.ok_or_else(|| syn::Error::new_spanned(item, "#[ores_graphql] requires explicit stream metadata; ores-stack cross-checks it against the semantic operation"))?;
    match (kind.as_str(), stream.as_str()) {
        ("query" | "mutation", "unary") | ("subscription", "server_stream") => {}
        ("subscription", _) => return Err(syn::Error::new_spanned(item, "GraphQL subscriptions require stream = \"server_stream\"")),
        (_, _) => return Err(syn::Error::new_spanned(item, "GraphQL queries and mutations require stream = \"unary\"")),
    }

    let _ = GRAPHQL_V1_HTTP_PATH;
    Ok(GraphqlProjection { operation_key, invoke, kind, field, stream })
}
...
fn string_value(expr: &Expr, field: &str) -> syn::Result<String> {
    match expr {
        ...
        _ => Err(syn::Error::new_spanned(expr, format!("#[ores_graphql] {field} must be a string literal"))),
    }
}

fn set_once<T>(slot: &mut Option<T>, value: T, span: impl quote::ToTokens, field: &str) -> syn::Result<()> {
    if slot.is_some() {
        return Err(syn::Error::new_spanned(span, format!("#[ores_graphql] {field} is given twice")));
    }
    *slot = Some(value);
    Ok(())
}
...
fn valid_operation_key(key: &str) -> bool {
    ...
    parts.len() >= 2 && parts.into_iter().all(|part| {
...
fn validate_graphql_name(value: &str) -> Result<(), String> {
    if value.starts_with("__") { return Err("GraphQL fields beginning with `__` are reserved for introspection".to_owned()); }
    let mut chars = value.chars();
    let Some(first) = chars.next() else { return Err("GraphQL field must not be empty".to_owned()); };
    if !(first == '_' || first.is_ascii_alphabetic()) { return Err(format!("GraphQL field {value:?} must start with ASCII letter or underscore")); }
    if chars.any(|character| !(character == '_' || character.is_ascii_alphanumeric())) { return Err(format!("GraphQL field {value:?} contains a character outside [_0-9A-Za-z]")); }
    Ok(())
}
...
mod tests {
    ...
    fn rejects_non_generated_invoker_path() {
        ...
        let args: Punctuated<Meta, Token![,]> = items.into_iter().collect();
        let item: ItemFn = parse_quote!(pub async fn resolver(ctx: Context) -> Result<(), Error> { todo!() });
        assert!(validate(&args, &item).unwrap_err().to_string().contains("__ores_invoke_"));
    }
