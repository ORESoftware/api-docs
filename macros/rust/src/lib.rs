...
pub fn ores_page(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    match validate_page(&args, &item) {
        Ok(meta) => {
            ...
            } else if let Some(tag) = meta.on_demand {
            ...
            quote! {
                ...
                /// Generated by `#[ores_page]`; consumed by generated router glue.
                #[doc(hidden)]
...
fn validate_page(
    ...
    for meta in args {
        let Meta::NameValue(value) = meta else {
            return Err(syn::Error::new_spanned(
                meta,
                "ores_page arguments must be key = value",
            ));
        };
        let key = value
            .path
        ...
        match key.as_str() {
            ...
            "revalidate_secs" => {
                revalidate_secs = Some(integer_value(value, "revalidate_secs")?)
            }
