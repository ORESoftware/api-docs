...
NODE_BUFFER = re.compile(r"(?<![A-Za-z0-9_$])Buffer(?![A-Za-z0-9_$])")
...
def require_member_order(
    keys: list[str], expected: tuple[str, ...], label: str
) -> None:
    require(set(keys) <= set(expected), f"{label}: unknown canonical member")
    positions = [expected.index(key) for key in keys]
    require(positions == sorted(positions), f"{label}: top-level member order drift")


def require_receipt_state(parsed: dict[str, Any], name: str) -> None:
    ok = parsed.get("ok")
    ...
    if status is not None:
        require(
            ...
            f"{name}: status type drift",
        )
        require(
            (200 <= status <= 399) if ok else (400 <= status <= 599),
            f"{name}: status/state mismatch",
        )
    require(
        ("error" not in parsed) if ok else ("error" in parsed),
        f"{name}: error/state mismatch",
    )
    require(ok or "body" not in parsed, f"{name}: failed receipt carries body")


def audit_fixture(path: str) -> None:
    fixture = load_object(path)
    require(fixture.get("schemaVersion") == 1, "fixture schema version drift")
    require(
        fixture.get("profile") == "ores-rpc-v1-call-receipt",
        "fixture profile drift",
    )
    require(
        fixture.get("maxFrameBytes") == 8 * 1024 * 1024,
        "fixture byte limit drift",
    )
    require(
        fixture.get("tcpLengthPrefixBytes") == 4,
        "fixture TCP prefix width drift",
    )
    ...
    for case in valid:
        ...
        require(
            len(encoded_bytes) <= 8 * 1024 * 1024,
            f"{name}: frame exceeds limit",
        )
        ...
        require_member_order(
            list(parsed), CALL_ORDER if kind == "call" else RECEIPT_ORDER, name
        )
        require(
            json.dumps(parsed, separators=(",", ":"), ensure_ascii=False) == encoded,
            f"{name}: encoded bytes are not canonical",
        )
        require(
            prefix == struct.pack(">I", len(encoded_bytes)).hex(),
            f"{name}: TCP length witness drift",
        )
        if kind == "receipt":
            require_receipt_state(parsed, name)
    ...
    for case in invalid:
        ...
        names.add(name)
        require(
            kind in {"call", "receipt"}, f"{name}: unknown invalid fixture kind"
        )
    ...
    require(
        valid_kinds == {"call", "receipt"},
        "valid corpus must cover call and receipt",
    )
    require(
        invalid_kinds == {"call", "receipt"},
        "invalid corpus must cover call and receipt",
    )
    require(
        any("ridl-v2" in name for name in names),
        "fixture lacks v1/v2 isolation case",
    )
...
def main() -> int:
    manifest = load_object("runtime/v1-conformance.json")
    require(manifest.get("schemaVersion") == 1, "unsupported RPC v1 manifest")
    require(
        manifest.get("profile") == "ores-rpc-v1-call-receipt",
        "unexpected RPC v1 profile",
    )
    require(
        manifest.get("authorities") == EXPECTED_AUTHORITIES,
        "RPC v1 authority inventory drift",
    )
    require(manifest.get("frameVersion") == 1, "RPC v1 version drift")
    require(
        manifest.get("maxFrameBytes") == 8 * 1024 * 1024,
        "RPC v1 byte limit drift",
    )
    require(
        manifest.get("tcpLengthPrefixBytes") == 4,
        "RPC v1 TCP prefix drift",
    )
    require(
    ...
    call_schema = load_object(EXPECTED_AUTHORITIES["jsonSchema"][0])
    receipt_schema = load_object(EXPECTED_AUTHORITIES["jsonSchema"][1])
    protobuf = read(EXPECTED_AUTHORITIES["protobuf"])
    for document, label in ((call_schema, "call"), (receipt_schema, "receipt")):
        require(
            document.get("$schema")
            == "https://json-schema.org/draft/2020-12/schema",
            f"{label} schema draft drift",
        )
        require(
            document.get("additionalProperties") is False,
            f"{label} schema must remain closed",
        )
    require(
        "status?: int32;" in typespec,
        "TypeSpec v1 status-presence contract drift",
    )
    require(
        "status" not in set(receipt_schema.get("required", [])),
        "JSON Schema v1 status became silently required",
    )
    require(
    ...
    require(
        tuple(sorted(languages)) == EXPECTED_LANGUAGES,
        "RPC v1 language set drift",
    )
    ...
    for language in EXPECTED_LANGUAGES:
        ...
        require(
            tuple(sources or ()) == EXPECTED_SOURCE_SETS[language],
            f"{language} source set drift",
        )
        require(
            isinstance(test_path, str) and test_path,
            f"{language} test path missing",
        )
        require(
            isinstance(command, str) and command,
            f"{language} command missing",
        )
        source_text = "\n".join(read(path) for path in sources)
        test_text = read(test_path)
        require(
            "rpc-v1" in test_text and "conformance.json" in test_text,
            f"{language} test is not fixture-bound",
        )
        require(
    ...
    require("export const RPC_VERSION = 1;" in typescript, "TypeScript version drift")
    require(
        "export const MAX_FRAME_BYTES = 8 * 1024 * 1024;" in typescript,
        "TypeScript byte limit drift",
    )
    for forbidden in ("node:", "process.", "require("):
        require(
            forbidden not in typescript,
            f"TypeScript RPC subpath is not browser-safe: {forbidden}",
        )
    require(
        NODE_BUFFER.search(typescript) is None,
        "TypeScript RPC subpath uses the Node Buffer global",
    )

    dart = sources_by_language["dart"]
    require("const int rpcV1Version = 1;" in dart, "Dart version drift")
    require(
        "const int rpcV1MaxFrameBytes = 8 * 1024 * 1024;" in dart,
        "Dart byte limit drift",
    )
    require("dart:io" not in dart, "Dart RPC runtime must not require dart:io")
    require(
        "package:" not in dart,
        "Dart RPC runtime must remain dependency-free",
    )

    go = sources_by_language["go"]
    require("RPCVersion        uint8 = 1" in go, "Go version drift")
    require(
        "MaxFrameBytes           = 8 * 1024 * 1024" in go,
        "Go byte limit drift",
    )
    imports = go_imports(go)
    require(
        imports <= ALLOWED_GO_IMPORTS,
        f"Go runtime import drift: {sorted(imports - ALLOWED_GO_IMPORTS)}",
    )

    rust = sources_by_language["rust"]
    require("pub const RPC_V1_VERSION: u32 = 1;" in rust, "Rust version drift")
    require("MAX_FRAME_BYTES" in rust, "Rust byte limit is not wired")
    rust_root = read("rust/src/lib.rs")
    require("pub mod rpc_v1;" in rust_root, "Rust RPC v1 module is not compiled")
    require(
        "RpcV1Receipt" in rust_root and "RpcV1Call" in rust_root,
        "Rust RPC v1 public exports missing",
    )
    ...
    for legacy in ("compileValidator", "parseRouteMap", "inferMethods", "MAX_FRAME_BYTES"):
        require(
            f"export function {legacy}" in typescript_root
            or f"export const {legacy}" in typescript_root,
            f"legacy TypeScript export lost: {legacy}",
        )
    for strict_export in (
        "decodeRpcV1Call",
        "decodeRpcV1Receipt",
        "RPC_V1_MAX_FRAME_BYTES",
    ):
        require(
            strict_export in typescript_root,
            f"strict TypeScript root alias missing: {strict_export}",
        )
