...
final class Rules {
  ...
  const Rules({this.kind = Kind.text, this.required = false, this.nonBlank = false,
    this.minChars, this.maxChars, this.minLines, this.maxLines,
    this.minimum, this.maximum, this.dateMin, this.dateMax});
}
...
final class FieldValidator {
  ...
  factory FieldValidator(Rules rules) {
    ...
    if (!ordered(rules.minChars, rules.maxChars) || !ordered(rules.minLines, rules.maxLines) ||
        [rules.minChars, rules.maxChars].nonNulls.any((n) => n < 0 || n > maxInputBytes) ||
        [rules.minLines, rules.maxLines].nonNulls.any((n) => n < 1 || n > maxInputBytes + 1)) {
    ...
    if ((!numeric && (rules.minimum != null || rules.maximum != null)) ||
        ...
        (rules.minimum != null && rules.maximum != null && rules.minimum! > rules.maximum!)) {
      throw const InvalidRules();
    }
    if (rules.kind == Kind.integer && [rules.minimum, rules.maximum].nonNulls.any((n) => n % 1 != 0 || n.abs() > maxSafeInteger)) {
      throw const InvalidRules();
    }
    if (rules.kind != Kind.date && (rules.dateMin != null || rules.dateMax != null)) {
      throw const InvalidRules();
    }
    final low = rules.dateMin == null ? null : _date(rules.dateMin!);
    final high = rules.dateMax == null ? null : _date(rules.dateMax!);
    if ((rules.dateMin != null && low == null) || (rules.dateMax != null && high == null) ||
  ...
  List<String> validate(String? value) {
    if (value == null || value.isEmpty) return rules.required ? const ['required'] : const [];
    ...
    if (value.length > maxInputBytes || utf8.encode(value).length > maxInputBytes) return const ['too_large'];
    final errors = <String>[];
    if (rules.nonBlank && value.runes.every(_whiteSpace)) errors.add('blank');
    final chars = value.runes.length;
    if (rules.minChars != null && chars < rules.minChars!) errors.add('min_chars');
    if (rules.maxChars != null && chars > rules.maxChars!) errors.add('max_chars');
    final lines = logicalLines(value);
    if (rules.minLines != null && lines < rules.minLines!) errors.add('min_lines');
    if (rules.maxLines != null && lines > rules.maxLines!) errors.add('max_lines');
    switch (rules.kind) {
      case Kind.text:
        break;
      case Kind.email:
      ...
      case Kind.integer:
        ...
        final parsed = _full(integer ? _integerPattern : _numberPattern, value) ? double.tryParse(value) : null;
        if (parsed == null || !parsed.isFinite) {
          errors.add(integer ? 'integer' : 'number');
        } else if (integer && parsed.abs() > maxSafeInteger) {
          errors.add('unsafe_integer');
        } else {
          if (rules.minimum != null && parsed < rules.minimum!) errors.add('minimum');
          if (rules.maximum != null && parsed > rules.maximum!) errors.add('maximum');
        }
      case Kind.date:
        ...
        if (parsed == null) {
          ...
          if (_dateMin != null && parsed.isBefore(_dateMin)) errors.add('date_min');
          if (_dateMax != null && parsed.isAfter(_dateMax)) errors.add('date_max');
        }
    }
  ...
  String? Function(String?) validator({String Function(String code)? localize}) => (value) {
...
bool _whiteSpace(int c) => (c >= 9 && c <= 13) || c == 32 || c == 133 || c == 160 || c == 5760 ||
...
int logicalLines(String value) {
  ...
  for (final c in value.runes) {
    if ((c == 13 || c == 10 || c == 133 || c == 8232 || c == 8233) && !(c == 10 && previousCr)) lines++;
    previousCr = c == 13;
  }
  return lines;
}
...
final _emailPattern = RegExp(r"[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*");
bool _email(String value) {
  ...
  return !local.startsWith('.') && !local.endsWith('.') && !local.contains('..') && domain.contains('.') && _full(_emailPattern, value);
}
...
final class FieldState {
  ...
  void edit(FieldValidator validator, String? value) { _errors = validator.validate(value); }
  void blur(FieldValidator validator, String? value) { _touched = true; edit(validator, value); }
