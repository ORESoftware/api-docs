...
bool _validField(String value) {
  ...
  bool alnum(int c) => (c >= 48 && c <= 57) || (c >= 65 && c <= 90) || (c >= 97 && c <= 122);
  return alnum(value.codeUnitAt(0)) && value.codeUnits.every((c) => alnum(c) || c == 95 || c == 46 || c == 58 || c == 45);
}
...
final class ValidationIssue {
  ...
  factory ValidationIssue(String field, String code) {
    if (!_validField(field) || !validationCodes.contains(code)) throw const InvalidValidationMessage();
  ...
  factory ValidationIssue.fromJson(Object? value) {
    ...
    if (field is! String || code is! String) throw const InvalidValidationMessage();
...
final class ValidationMessage {
  ...
  factory ValidationMessage.fromCodes(String field, List<String> codes) {
    if (!_validField(field) || codes.length > 128) throw const InvalidValidationMessage();
    return ValidationMessage(codes.map((code) => ValidationIssue(field, code)).toList());
  }
  factory ValidationMessage.fromJson(Object? value) {
    final map = _object(value, {'schema_version', 'issues'});
    final issues = map['issues'];
    if (map['schema_version'] != validationMessageVersion || issues is! List || issues.length > 128) {
      throw const InvalidValidationMessage();
    }
