...
fn invalid_configuration_never_becomes_a_permissive_validator() {
    let cases = [
        ...
        Rules {
            kind: Kind::Date,
            date_min: Some("2023-02-29".into()),
            ..Rules::default()
        },
        Rules {
            kind: Kind::Date,
            date_min: Some("2025-01-01".into()),
            date_max: Some("2024-01-01".into()),
            ..Rules::default()
        },
        Rules {
            min_chars: Some(MAX_INPUT_BYTES + 1),
            ..Rules::default()
        },
    ];
    for rules in cases {
        assert!(FieldValidator::new(rules).is_err());
    }
}
...
fn input_budget_and_numeric_overflow() {
    ...
    assert_eq!(
        text.validate(Some(&"a".repeat(MAX_INPUT_BYTES + 1)))[0].as_str(),
        "too_large"
    );
    assert_eq!(
        text.validate(Some(&"😀".repeat(MAX_INPUT_BYTES / 4 + 1)))[0].as_str(),
        "too_large"
    );
    let number = FieldValidator::new(Rules {
        kind: Kind::Number,
        ..Rules::default()
    })
    .unwrap();
    assert_eq!(
        number.validate(Some(&"9".repeat(400)))[0].as_str(),
        "number"
    );
}
...
fn serde_and_garde_compose_without_treating_decode_as_validation() {
    ...
    struct Input {
        #[garde(custom(garde_rule))]
        value: String,
    }
    let rules = FieldValidator::new(Rules {
        kind: Kind::Email,
        required: true,
        ..Rules::default()
    })
