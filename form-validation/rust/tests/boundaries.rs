...
fn email_mailbox_and_label_limits() {
    ...
    assert!(email
        .validate(Some(&format!("{}@example.com", "a".repeat(64))))
        .is_empty());
    assert_eq!(
        email.validate(Some(&format!("{}@example.com", "a".repeat(65)))),
        [Code::Email]
    );
...
fn separators_and_scalar_budgets_are_exact() {
    let two = FieldValidator::new(Rules {
        min_lines: Some(2),
        max_lines: Some(2),
        required: true,
        ..Rules::default()
    ...
    let four = FieldValidator::new(Rules {
        max_chars: Some(4),
        ..Rules::default()
    })
    .unwrap();
    ...
    assert!(text
        .validate(Some(&"😀".repeat(MAX_INPUT_BYTES / 4)))
        .is_empty());
    assert_eq!(
        text.validate(Some(&"😀".repeat(MAX_INPUT_BYTES / 4 + 1))),
        [Code::TooLarge]
    );
}
