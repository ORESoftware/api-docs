...
impl FieldValidator {
    pub fn new(rules: Rules) -> Result<Self, InvalidRules> {
        ...
        if !ordered(rules.min_chars, rules.max_chars)
            ...
            || [rules.min_chars, rules.max_chars].into_iter().flatten().any(|n| n > MAX_INPUT_BYTES)
            || [rules.min_lines, rules.max_lines].into_iter().flatten().any(|n| n == 0 || n > MAX_INPUT_BYTES + 1)
        {
        ...
        if (!numeric && (rules.minimum.is_some() || rules.maximum.is_some()))
            || [rules.minimum, rules.maximum].into_iter().flatten().any(|n| !n.is_finite())
            || matches!((rules.minimum, rules.maximum), (Some(a), Some(b)) if a > b)
        {
            return Err(InvalidRules);
        }
        if rules.kind == Kind::Integer
            && [rules.minimum, rules.maximum].into_iter().flatten().any(|n| n.fract() != 0.0 || n.abs() > MAX_SAFE_INTEGER)
        {
            return Err(InvalidRules);
        }
        ...
        let date_min = rules.date_min.as_deref().map(parse_date).transpose_option()?;
        let date_max = rules.date_max.as_deref().map(parse_date).transpose_option()?;
        if matches!((date_min, date_max), (Some(a), Some(b)) if a > b) {
            return Err(InvalidRules);
        }
        Ok(Self { rules, date_min, date_max })
    }
    ...
    pub fn validate(&self, value: Option<&str>) -> Vec<Code> {
        let Some(value) = value.filter(|s| !s.is_empty()) else {
            return if self.rules.required { vec![Code::Required] } else { vec![] };
        };
        ...
        if self.rules.min_chars.is_some_and(|n| chars < n) { errors.push(Code::MinChars); }
        if self.rules.max_chars.is_some_and(|n| chars > n) { errors.push(Code::MaxChars); }
        let lines = logical_lines(value);
        if self.rules.min_lines.is_some_and(|n| lines < n) { errors.push(Code::MinLines); }
        if self.rules.max_lines.is_some_and(|n| lines > n) { errors.push(Code::MaxLines); }
        match self.rules.kind {
            Kind::Text => {}
            Kind::Email => if !email(value) { errors.push(Code::Email); },
            Kind::PhoneE164 => if !phone_e164(value) { errors.push(Code::PhoneE164); },
            Kind::Number | Kind::Integer => {
                ...
                match number {
                    ...
                    Some(n) if integer && n.abs() > MAX_SAFE_INTEGER => errors.push(Code::UnsafeInteger),
                    Some(n) => {
                        if self.rules.minimum.is_some_and(|min| n < min) { errors.push(Code::Minimum); }
                        if self.rules.maximum.is_some_and(|max| n > max) { errors.push(Code::Maximum); }
                    }
                }
            }
            Kind::Date => match parse_date(value) {
                ...
                Some(date) => {
                    if self.date_min.is_some_and(|min| date < min) { errors.push(Code::DateMin); }
                    if self.date_max.is_some_and(|max| date > max) { errors.push(Code::DateMax); }
                }
            },
        }
        errors
...
trait TransposeOption<T> { fn transpose_option(self) -> Result<Option<T>, InvalidRules>; }
impl<T> TransposeOption<T> for Option<Option<T>> {
    fn transpose_option(self) -> Result<Option<T>, InvalidRules> {
        match self { None => Ok(None), Some(Some(value)) => Ok(Some(value)), Some(None) => Err(InvalidRules) }
    }
}
...
struct HtmlEmail<'a> { #[garde(email)] value: &'a str }
fn email(value: &str) -> bool {
    if !value.is_ascii() || value.len() > 254 { return false; }
    let Some((local, domain)) = value.split_once('@') else { return false; };
    !local.is_empty() && local.len() <= 64 && !local.starts_with('.') && !local.ends_with('.')
        && !local.contains("..") && domain.contains('.') && HtmlEmail { value }.validate().is_ok()
}
fn phone_e164(value: &str) -> bool {
    let b = value.as_bytes();
    (3..=16).contains(&b.len()) && b[0] == b'+' && matches!(b[1], b'1'..=b'9') && b[2..].iter().all(u8::is_ascii_digit)
}
fn decimal(value: &str, integer: bool) -> Option<f64> {
    let unsigned = value.strip_prefix('-').unwrap_or(value);
    let (whole, fraction) = unsigned.split_once('.').map_or((unsigned, None), |(a, b)| (a, Some(b)));
    if whole.is_empty() || !whole.bytes().all(|b| b.is_ascii_digit()) || (whole.len() > 1 && whole.starts_with('0'))
        || (integer && fraction.is_some()) || fraction.is_some_and(|s| s.is_empty() || !s.bytes().all(|b| b.is_ascii_digit()))
    { return None; }
    value.parse::<f64>().ok().filter(|n| n.is_finite())
}
fn parse_date(value: &str) -> Option<NaiveDate> {
    let b = value.as_bytes();
    if b.len() != 10 || b[4] != b'-' || b[7] != b'-' || b.iter().enumerate().any(|(i, c)| i != 4 && i != 7 && !c.is_ascii_digit()) {
        return None;
    }
...
pub struct FieldState { touched: bool, submitted: bool, errors: Vec<Code> }
impl FieldState {
    ...
    pub fn visible_errors(&self) -> &[Code] {
        if self.touched || self.submitted { &self.errors } else { &[] }
    }
}
