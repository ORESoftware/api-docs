...
fn main() {
    assert_eq!(
        std::env::args_os().len(),
        1,
        "fixed probe accepts no arguments"
    );
    let mut input = String::new();
    io::stdin()
        .take(1_048_577)
        .read_to_string(&mut input)
        .expect("read synthetic input");
    ...
    let messages: Vec<_> = input
        ...
        .into_iter()
        .map(
            |case| match serde_json::from_value::<ValidationMessage>(case.instance) {
    ...
    let fields: Vec<_> = input
        ...
        .map(|case| {
            ...
            let message =
                ValidationMessage::from_codes("input", &codes).expect("known core errors");
