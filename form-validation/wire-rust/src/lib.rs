...
pub const CODES: [&str; 18] = [
    "too_large", "required", "blank", "min_chars", "max_chars", "min_lines",
    "max_lines", "email", "phone_e164", "number", "integer", "unsafe_integer",
    "minimum", "maximum", "date", "date_min", "date_max", "invalid_unicode",
];
...
fn valid_field(field: &str) -> bool {
    ...
    (1..=128).contains(&bytes.len()) && bytes[0].is_ascii_alphanumeric()
...
impl ValidationIssue {
    pub fn new(field: &str, code: &str) -> Result<Self, InvalidMessage> {
        if !valid_field(field) || !CODES.contains(&code) { return Err(InvalidMessage); }
        Ok(Self { field: field.to_owned(), code: code.to_owned() })
    }
...
struct RawIssue { field: String, code: String }
impl TryFrom<RawIssue> for ValidationIssue {
    type Error = InvalidMessage;
    fn try_from(raw: RawIssue) -> Result<Self, Self::Error> { Self::new(&raw.field, &raw.code) }
}
impl From<ValidationIssue> for RawIssue {
    fn from(issue: ValidationIssue) -> Self { Self { field: issue.field, code: issue.code } }
}

/// Empty issues means no local errors, never authorization or server acceptance.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(try_from = "RawMessage", into = "RawMessage")]
pub struct ValidationMessage { issues: Vec<ValidationIssue> }
impl ValidationMessage {
    pub fn new(issues: Vec<ValidationIssue>) -> Result<Self, InvalidMessage> {
        if issues.len() > MAX_ISSUES { return Err(InvalidMessage); }
        Ok(Self { issues })
    }
    pub fn from_codes(field: &str, codes: &[Code]) -> Result<Self, InvalidMessage> {
        if !valid_field(field) || codes.len() > MAX_ISSUES { return Err(InvalidMessage); }
        Self::new(codes.iter().map(|&code| ValidationIssue::from_code(field, code)).collect::<Result<_, _>>()?)
    }
    pub fn issues(&self) -> &[ValidationIssue] { &self.issues }
}
#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct RawMessage { schema_version: String, issues: Vec<ValidationIssue> }
impl TryFrom<RawMessage> for ValidationMessage {
    type Error = InvalidMessage;
    fn try_from(raw: RawMessage) -> Result<Self, Self::Error> {
        if raw.schema_version != SCHEMA_VERSION { return Err(InvalidMessage); }
        Self::new(raw.issues)
    }
}
impl From<ValidationMessage> for RawMessage {
    fn from(message: ValidationMessage) -> Self {
        Self { schema_version: SCHEMA_VERSION.to_owned(), issues: message.issues }
    }
}
...
mod tests {
    ...
    fn actual_validator_outputs_round_trip_without_values() {
        let validator = FieldValidator::new(Rules { kind: Kind::Email, required: true, ..Rules::default() }).unwrap();
        let message = ValidationMessage::from_codes("email", &validator.validate(Some("private-sentinel"))).unwrap();
        let encoded = serde_json::to_string(&message).unwrap();
        assert!(!encoded.contains("private-sentinel"));
        assert_eq!(serde_json::from_str::<ValidationMessage>(&encoded).unwrap(), message);
    ...
    fn rejects_unknown_fields_codes_versions_and_limits() {
        ...
        for code in CODES { assert!(ValidationIssue::new("field", code).is_ok()); }
    }
}
