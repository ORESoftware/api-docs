...
impl ModuleInterfaceSpec {
    ...
    fn validate(&self) -> Result<(), ModuleInterfaceCodegenError> {
        ...
        if self.contract_id.chars().any(char::is_whitespace) {
            return Err(ModuleInterfaceCodegenError::InvalidContractId);
        }
...
pub fn render_module_interface(
    ...
    let file_name = file_name(language, spec);
    let source = render_source(language, spec);
...
fn generated_header(spec: &ModuleInterfaceSpec, comment_prefix: &str) -> String {
    return format!(
        "{comment_prefix} @generated by ORESoftware/api-docs module-interface-codegen\n{comment_prefix} contract: {}\n",
        spec.contract_id
    );
}
...
mod tests {
    ...
    fn spec() -> ModuleInterfaceSpec {
        return ModuleInterfaceSpec::new(
            "catalog_worker",
            "handle",
    ...
    fn matrix_is_deterministic_and_complete_for_scintilla() {
        ...
        for artifact in first {
            ...
            assert!(artifact.source.contains("ores-stack.worker.v1"));
    ...
    fn beamscale_fails_closed_for_non_beam_projections() {
        ...
        .expect_err("BeamScale must reject Rust as a direct guest projection");
    ...
    fn native_module_families_render_native_contract_syntax() {
        let cases = [
            (ModuleInterfaceLanguage::StandardMl, "signature CATALOG_WORKER_MODULE"),
            (ModuleInterfaceLanguage::Ocaml, "module type CATALOG_WORKER_MODULE"),
            (ModuleInterfaceLanguage::Ada, "package Catalog_Worker is"),
            (ModuleInterfaceLanguage::Modula2, "DEFINITION MODULE CatalogWorker"),
            (ModuleInterfaceLanguage::Modula3, "INTERFACE CatalogWorker"),
            (ModuleInterfaceLanguage::Haskell, "signature CatalogWorker where"),
    ...
    fn portable_identifier_validation_is_fail_closed() {
        let invalid = ModuleInterfaceSpec::new(
            "../../module",
            "handle",
            "ores-stack.worker.v1",
        );
