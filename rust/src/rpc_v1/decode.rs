pub fn decode_rpc_v1_call(payload: &[u8]) -> Result<RpcV1Call, SchemaError> {
    ...
    if version != u64::from(RPC_V1_VERSION) { return instance("rpc-call", format!("unsupported RPC version {version}")); }
    if required_string(&value, "op", "rpc-call")? != "call" { return instance("rpc-call", "expected op call"); }
    let call = RpcV1Call {
        ...
        body: value.get("body").cloned().map_or_else(OptionalJson::absent, OptionalJson::present),
...
pub fn decode_rpc_v1_receipt(payload: &[u8]) -> Result<RpcV1Receipt, SchemaError> {
    ...
    if version != u64::from(RPC_V1_VERSION) { return instance("rpc-receipt", format!("unsupported RPC version {version}")); }
    if required_string(&value, "op", "rpc-receipt")? != "receipt" { return instance("rpc-receipt", "expected op receipt"); }
    let status = match value.get("status") {
        None => None,
        Some(raw) => Some(raw.as_u64().and_then(|value| u16::try_from(value).ok())
    ...
    let receipt = RpcV1Receipt {
        ...
        ok: value.get("ok").and_then(Value::as_bool).ok_or_else(|| schema_error("rpc-receipt", "ok has the wrong type"))?,
        status,
        body: value.get("body").cloned().map_or_else(OptionalJson::absent, OptionalJson::present),
...
pub fn assert_rpc_v1_receipt_for_call(call: &RpcV1Call, receipt: &RpcV1Receipt) -> Result<(), SchemaError> {
    call.validate()?;
    receipt.validate()?;
    if receipt.id != call.id { return instance("rpc-receipt", "receipt id does not match call id"); }
    if receipt.key != call.key { return instance("rpc-receipt", "receipt key does not match call key"); }
    if let (Some(left), Some(right)) = (call.transport, receipt.transport) {
        if left != right { return instance("rpc-receipt", "receipt transport does not match call transport"); }
    }
    Ok(())
}
...
pub struct RpcV1Correlator { prefix: String, next: u64 }

impl RpcV1Correlator {
    pub fn new(prefix: impl Into<String>) -> Result<Self, SchemaError> {
        let prefix = prefix.into();
        if prefix.chars().count() >= 128 { return instance("rpc-call", "correlation prefix must contain fewer than 128 Unicode scalars"); }
        Ok(Self { prefix, next: 0 })
    }

    pub fn take(&mut self) -> Result<String, SchemaError> {
        self.next = self.next.checked_add(1).ok_or_else(|| schema_error("rpc-call", "correlation id counter exhausted"))?;
