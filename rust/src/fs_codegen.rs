...
/// forbids `generate_static_params` from living in `page.rs`. The attribute
/// macro emits a typed `__ORES_PAGE_CONFIG` constant, so generated glue can
/// verify the static metadata and exact page signature with rustc. Optional
/// sibling `gen.rs` is analyzed independently and type-checked the same way.
