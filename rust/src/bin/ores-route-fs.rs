...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parsed = sources.into_iter().map(|source| match kind {
    ...
    let entries = routes.into_iter().map(|route| {
        let canonical_path = route.canonical_path();
        let api_operations = route_map.as_ref().map(|map| {
            map.map.iter()
        ...
        if kind == FsRouteKind::ApiHandler && api_operations.is_empty() {
            return Err(format!("{} derives {} but no api-docs route-map operation owns that path", route.source, canonical_path));
        }
        Ok(Entry {
...
fn collect(repo_root: &Path, dir: &Path, leaf: &str, out: &mut Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
