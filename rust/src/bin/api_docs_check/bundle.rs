...
use super::common::{
    read_json, read_text, sha256_json, write_json, write_text, CheckResult, TempDir,
};
use super::routes::{
...
fn gen_go(contract: &Value) -> CheckResult<String> {
    ...
    for operation in operations {
        ...
        lines.extend([
            ...
            format!(
                "\t\tPath:              {},",
                json_string_literal(string_field("path"))?
            ),
            ...
            format!(
                "\t\tDelivery:          {},",
                json_string_literal(string_field("delivery"))?
            ),
            format!(
                "\t\tAliasOf:           {},",
                json_string_literal(string_field("aliasOf"))?
            ),
            format!(
                "\t\tOptoSyncTable:     {},",
                json_string_literal(
...
pub fn verify_bundle(target: &Path, map: &RouteMap, map_path: &Path) -> CheckResult<()> {
    ...
    let expected_docs = [
        (
            "openapi",
            project::openapi(map).map_err(|error| error.to_string())?,
        ),
        (
            "openrpc",
            project::openrpc(map).map_err(|error| error.to_string())?,
        ),
        (
            "connect",
            project::connect(map).map_err(|error| error.to_string())?,
        ),
        (
            "hyper-schema",
    ...
    let expected_languages = [
        ...
        (
            "rust/routes.rs",
            normalize_source(gen_rust_bound(&contract, &ordered, map)?),
        ),
        (
            "dart/routes.dart",
            normalize_source(gen_dart_bound(&contract, &ordered, map)?),
        ),
        (
            "gleam/routes.gleam",
...
mod tests {
    ...
    fn document_drift_is_a_veto() {
        ...
        assert!(verify_bundle(&target, &map, &source)
            .unwrap_err()
            .contains("openapi"));
    }
