...
pub trait FramedRpcStream {
    ...
    fn open(&self, call: RpcStreamCall) -> Result<RpcStreamSession<Self::Error>, Self::Error>;
}
...
impl std::fmt::Display for RpcStreamPrepareError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::OperationNotAllowed(key) => write!(
                formatter,
                "RPC stream operation not generated for this audience: {key}"
            ),
        }
    }
}
    ...
    F: Fn(serde_json::Value) -> Result<T, String>,
{
    ...
    pub fn add_body_field(mut self, name: impl Into<String>, value: impl serde::Serialize) -> Self {
            ...
            .and_then(serde_json::Value::as_object_mut)
        {
            ...
            self.protocol_error = Some("add_body_field requires an object RPC body".to_owned());
        }
        self
    }

    /// Sole transport-open / network-I/O boundary.
    pub fn stream(self) -> Result<RpcStreamClient<S::Error, T, F>, RpcStreamError<S::Error>> {
