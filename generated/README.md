...
- `cargo run --quiet --locked --manifest-path rust/Cargo.toml --bin api-docs-check -- generate-routes` (TypeScript, Dart, Rust, Gleam `RouteKey` objects from v1 `examples/*.route-map.json`)
...
The legacy `scripts/generate-routes.py` entrypoint remains only for compatibility while consumers migrate; it is not an official CI or zed-pkg invocation.

```sh
cargo run --quiet --locked --manifest-path rust/Cargo.toml --bin api-docs-check -- generate-routes
